...
export default function CrearCapacitacionPage() {
    ...
    return (
        <div className="max-w-6xl mx-auto space-y-6 pb-20 relative">
            ...
            {/* 🟢 BANNER DE ALERTAS FLOTANTE (Siempre visible) */}
            {mensajeAlerta && (
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4">
                    <div className="bg-white rounded-xl shadow-2xl w-full max-w-md overflow-hidden">
                        <div className="bg-gray-100 px-4 py-3 border-b flex justify-between items-center">
                            ...
                            <button type="button" onClick={cerrarModalFirma}><X size={24} className="text-gray-500 hover:text-red-500" /></button>
                        </div>
                        ...
                        <div className="p-4 border-t bg-gray-50 flex justify-end gap-3">
                            <button type="button" onClick={() => workerPadRef.current?.clear()} className="px-4 py-2 text-gray-600 rounded-lg text-sm">Limpiar</button>
                            <button type="button" onClick={guardarFirmaModal} className="px-6 py-2 bg-blue-600 text-white rounded-lg font-bold">Aceptar</button>
                        </div>
                    </div>
                </div>
            )}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <div className="flex gap-2">
                                <input {...register("codigo_acta", { required: "Este campo es obligatorio" })} className={`w-full border rounded px-3 py-2 font-mono outline-none transition-all ${errors.codigo_acta ? 'border-red-500 bg-red-50 ring-2 ring-red-200 text-red-900' : 'bg-gray-50 text-blue-900 focus:ring-2 focus:ring-blue-500'}`} placeholder="ACT-2025-XXX" />
                                <button
                                    type="button"
                                    onClick={generarCodigoActa}
                                    className="px-4 py-2 bg-blue-100 hover:bg-blue-200 text-blue-700 font-bold text-sm rounded-lg transition-colors border border-blue-200"
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
                        <div className="md:col-span-8 relative" ref={autocompleteRef}>
                            ...
                            <div className="relative">
                                <input {...temaRegister} onChange={(e) => { temaRegister.onChange(e); handleTemaSearch(e.target.value); }} className={`w-full border rounded pl-9 pr-3 py-2 outline-none transition-all ${errors.tema_principal ? 'border-red-500 bg-red-50 ring-2 ring-red-200 text-red-900' : 'focus:ring-2 focus:ring-blue-500'}`} placeholder="Buscar tema..." autoComplete="off" />
                                            ...
                                            {plan.areas_objetivo && (
                                                <div className="flex flex-wrap gap-1.5">
                                                    {plan.areas_objetivo.split(',').map((area, i) => (
                                                        <span key={i} className="text-[10px] uppercase font-bold bg-blue-100 text-blue-800 px-2 py-0.5 rounded border border-blue-200">
                                                            {area.trim()}
                                                        </span>
                                                    ))}
                                                </div>
                        ...
                        <div className="md:col-span-3">
                            ...
                            <input type="date" {...register("fecha", { required: true })} className={`w-full border rounded px-3 py-2 outline-none transition-all ${errors.fecha ? 'border-red-500 bg-red-50 ring-2 ring-red-200 text-red-900' : 'focus:ring-2 focus:ring-blue-500'}`} />
                ...
                <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 text-sm">
                        <div>
                            ...
                            <div className={`grid grid-cols-2 gap-2 p-3 rounded-lg border transition-all ${errors.actividad ? 'bg-red-50 border-red-300 ring-2 ring-red-200' : 'border-transparent'}`}>
                        ...
                        <div>
                            ...
                            <select {...register("categoria", { required: true })} className={`w-full border rounded px-3 py-2 outline-none transition-all ${errors.categoria ? 'border-red-500 bg-red-50 ring-2 ring-red-200 text-red-900' : 'focus:ring-2 focus:ring-blue-500'}`}>
                        ...
                        <div className="flex gap-8">
                            <div>
                                ...
                                <div className={`flex gap-3 p-2 rounded-lg border transition-all ${errors.modalidad ? 'bg-red-50 border-red-300 ring-2 ring-red-200' : 'border-transparent'}`}>
                                    <label><input type="radio" value="Interna" {...register("modalidad", { required: true })} className="accent-blue-600" /> Interna</label>
                                    <label><input type="radio" value="Externa" {...register("modalidad", { required: true })} className="accent-blue-600" /> Externa</label>
                                </div>
                            </div>
                            <div>
                                ...
                                <div className={`flex gap-3 p-2 rounded-lg border transition-all ${errors.accion_correctiva ? 'bg-red-50 border-red-300 ring-2 ring-red-200' : 'border-transparent'}`}>
                                    <label><input type="radio" value="SI" {...register("accion_correctiva", { required: true })} className="accent-blue-600" /> SI</label>
                                    <label><input type="radio" value="NO" {...register("accion_correctiva", { required: true })} className="accent-blue-600" /> NO</label>
                                </div>
                            </div>
                        </div>
                        <div>
                            ...
                            <div className={`flex flex-wrap gap-3 p-3 rounded-lg border transition-all ${errors.centros ? 'bg-red-50 border-red-300 ring-2 ring-red-200' : 'border-transparent'}`}>
                                {['Planta Packing', 'Fundo', 'Campo', 'Auditorio', 'Otros'].map(c => (
                                    <label key={c} className="flex gap-1"><input type="radio" value={c} {...register("centros", { required: true })} className="accent-blue-600" /> {c}</label>
                                ))}
                            </div>
                            ...
                            <button
                                ...
                                onClick={() => {
                                    if (confirm("¿Estás seguro de vaciar toda la lista?")) {
                                        replace([{ numero: 1, dni: '', apellidos_nombres: '', area: '', cargo: '', genero: 'M', condicion: '' }]);
                                    }
                                }}
