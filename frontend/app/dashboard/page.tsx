...
export default function DashboardPage() {
    ...
    const handleToggleEstado = async (id: number, estadoActual: boolean) => {
        try {
            ...
            setEmpresasSaaS(prev => prev.map(emp =>
                emp.id_empresa === id ? { ...emp, estado: !estadoActual } : emp
            ));
        } catch (error) {
            ...
            try {
                if (userRole === 'soporte') {
                    const [resStats, resEmpresas] = await Promise.all([
                        ...
                        api.get('/dashboard/saas/empresas')
                    ]);
                    setStats(resStats.data);
                    setEmpresasSaaS(resEmpresas.data);
                } else {
    ...
    return (
        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
            ...
            <div className="bg-gradient-to-r from-blue-50 to-indigo-50/30 dark:from-slate-800 dark:to-slate-800/80 p-8 rounded-3xl border border-blue-100/50 dark:border-slate-700 flex flex-col md:flex-row justify-between items-center gap-4 shadow-sm">
                <div>
                    ...
                    <p className="text-blue-600/70 dark:text-blue-400/80 text-sm mt-2 font-medium">
                        {userRole === 'soporte'
                            ? 'Panel de control SaaS y monitoreo global de clientes.'
                            : 'Resumen general del sistema de capacitaciones.'}
                    </p>
                </div>

                {userRole !== 'auditor' && userRole !== 'soporte' && (
                    <Link
                    ...
                    <>
                        <div className="bg-white dark:bg-slate-800 p-6 rounded-3xl shadow-sm hover:shadow-xl hover:shadow-gray-200/40 dark:hover:shadow-slate-900/40 border border-gray-100/60 dark:border-slate-700 hover:-translate-y-1 transition-all duration-300 group">
                            <div className="flex justify-between items-start">
                                <div>
                                    <p className="text-gray-400 dark:text-gray-500 text-sm font-semibold tracking-wide uppercase">Total Empresas</p>
                                    <h3 className="text-4xl font-extrabold text-gray-800 dark:text-gray-100 mt-2 tracking-tight group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors">{stats.totalEmpresas || 1}</h3>
                                </div>
                                <div className="p-3 bg-gradient-to-br from-blue-50 to-blue-100 dark:from-blue-900/40 dark:to-blue-800/40 text-blue-600 dark:text-blue-400 rounded-2xl shadow-inner group-hover:scale-110 transition-transform"><Building2 size={24} /></div>
                            </div>
                        </div>
                        ...
                        <div className="bg-white dark:bg-slate-800 p-6 rounded-3xl shadow-sm hover:shadow-xl hover:shadow-gray-200/40 dark:hover:shadow-slate-900/40 border border-gray-100/60 dark:border-slate-700 hover:-translate-y-1 transition-all duration-300 group">
                            <div className="flex justify-between items-start">
                                <div>
                                    <p className="text-gray-400 dark:text-gray-500 text-sm font-semibold tracking-wide uppercase">Total Trabajadores</p>
                                    <h3 className="text-4xl font-extrabold text-gray-800 dark:text-gray-100 mt-2 tracking-tight group-hover:text-amber-500 dark:group-hover:text-amber-400 transition-colors">{stats.totalTrabajadores || 0}</h3>
                                </div>
                                <div className="p-3 bg-gradient-to-br from-amber-50 to-amber-100 dark:from-amber-900/40 dark:to-amber-800/40 text-amber-500 dark:text-amber-400 rounded-2xl shadow-inner group-hover:scale-110 transition-transform"><HardHat size={24} /></div>
                            </div>
                        </div>
                    </>
                ) : (
            ...
            {/* --- GRÁFICO DE BARRAS POR ÁREA (OCULTO PARA SOPORTE) --- */}
            {userRole !== 'soporte' && (
            ...
            {/* --- MITAD INFERIOR: VISTA SOPORTE (SAAS) VS VISTA CLIENTE --- */}
            {userRole === 'soporte' ? (
                /* 🟢 MODO SAAS: TARJETAS DE EMPRESAS CLIENTES */
                <div className="space-y-6">
                    <h2 className="text-xl font-bold text-gray-800 dark:text-gray-100 flex items-center gap-3">
                        <div className="p-2 bg-purple-50 dark:bg-purple-900/40 text-purple-500 dark:text-purple-400 rounded-xl">
                            <Building2 size={20} />
                        </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {empresasSaaS.map((emp) => (
                            <div key={emp.id_empresa} className="bg-white dark:bg-slate-800 rounded-3xl p-6 border border-gray-100 dark:border-slate-700 shadow-sm hover:shadow-xl transition-all duration-300 group flex flex-col justify-between">

                                <div className="flex justify-between items-start mb-4">
                                    <div>
                                        <h3 className="font-bold text-lg text-gray-800 dark:text-gray-100 group-hover:text-blue-600 transition-colors line-clamp-1" title={emp.nombre}>
                                            {emp.nombre}
                                        </h3>
                                    ...
                                    <span className={`px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider shadow-sm ${emp.estado ? 'bg-green-100 text-green-700 border border-green-200' : 'bg-red-100 text-red-700 border border-red-200'}`}>
                                        {emp.estado ? 'Activo' : 'Suspendido'}
                                    </span>
                                </div>

                                <div className="bg-gray-50 dark:bg-slate-900/50 rounded-2xl p-4 mb-5 border border-gray-100 dark:border-slate-700/50">
                                    <p className="text-[10px] uppercase font-bold text-gray-400 mb-3 tracking-widest">Estructura de Usuarios</p>
                                    <div className="grid grid-cols-3 gap-2 text-center divide-x divide-gray-200 dark:divide-slate-700">
                                        <div>
                                            <p className="text-2xl font-black text-gray-700 dark:text-gray-200">{emp.admins}</p>
                                            <p className="text-[10px] font-medium text-gray-500 mt-1">Admins</p>
                                        </div>
                                        <div>
                                            <p className="text-2xl font-black text-gray-700 dark:text-gray-200">{emp.auditores}</p>
                                            <p className="text-[10px] font-medium text-gray-500 mt-1">Auditores</p>
                                        </div>
                                        <div>
                                            <p className="text-2xl font-black text-gray-700 dark:text-gray-200">{emp.supervisores}</p>
                                            <p className="text-[10px] font-medium text-gray-500 mt-1">Supervis</p>
                                        </div>
                                    </div>
                                </div>

                                <div className="flex justify-between items-center text-sm font-medium text-gray-500 mb-5 px-2">
                                    <div className="flex items-center gap-2">
                                        <Users size={16} className="text-amber-500" />
                                        {emp.total_trabajadores} Trab.
                                    </div>
                                    <div className="flex items-center gap-2">
                                        <FileText size={16} className="text-blue-500" />
                                        {emp.total_capacitaciones} Actas
                                    </div>
                                </div>

                                <div className="flex gap-3 mt-auto">
                                    <button
                                        onClick={() => router.push(`/dashboard/empresas/${emp.id_empresa}`)}
                                        className="flex-1 bg-white dark:bg-slate-700 border border-gray-200 dark:border-slate-600 text-gray-700 dark:text-gray-200 py-2.5 rounded-xl text-sm font-bold hover:bg-gray-50 dark:hover:bg-slate-600 transition-colors shadow-sm"
                                    >
                                        Ver Detalles
                                    </button>
                                    <button
                                        onClick={() => handleToggleEstado(emp.id_empresa, emp.estado)}
                                        className={`w-12 flex items-center justify-center rounded-xl border transition-colors shadow-sm ${emp.estado
                ...
                /* 🟢 MODO CLIENTE: TABLA DE CAPACITACIONES */
                    ...
                    <div className="overflow-x-auto bg-gray-50/20 dark:bg-slate-800/20">
                        {capacitaciones.length === 0 ? (
                                    ...
                                    {capacitacionesFiltradas.map((cap) => (
                                        <div key={cap.id_capacitacion} className="bg-white dark:bg-slate-800 p-5 rounded-2xl shadow-sm border border-gray-100 dark:border-slate-700 flex flex-col gap-3 relative overflow-hidden group hover:shadow-md transition-all">
                                            ...
                                            <div className="flex flex-col gap-1.5 mt-1 border-t border-gray-50 dark:border-slate-700/50 pt-3">
                                                ...
                                                <div className="flex items-center gap-2 text-sm text-gray-600 dark:text-gray-400 font-medium">
                                                    <MapPin size={16} className={cap.sede_empresa === 'Olmos' ? 'text-orange-400 shrink-0' : 'text-green-400 shrink-0'} />
                            ...
                            <div className="p-12 text-center text-gray-500 dark:text-gray-400 bg-gray-50/30 dark:bg-slate-800/30">
                                ...
                                <p className="mt-1 font-bold text-blue-600 dark:text-blue-400 wrap-break-words">&quot;{busqueda}&quot;</p>
                            </div>
                        )}
                    </div>
                </div>
