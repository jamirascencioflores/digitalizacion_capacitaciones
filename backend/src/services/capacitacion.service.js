...
class CapacitacionService {
    /**
     * Obtiene todas las capacitaciones filtradas por rol/usuario
     */
    async getAll(id_auth, rol_auth) {
    ...
    async create(data, usuarioId) {
        const { participantes, documentos, ...resto } = data;
    ...
    async getById(id) {
        ...
        if (capacitacion.area_objetivo) {
            const areasParaBuscar = capacitacion.area_objetivo.split(",").map(normalizar);
            const todosTrabajadores = await prisma.trabajadores.findMany({ where: { estado: true } });

            const trabajadoresObjetivo = todosTrabajadores.filter((t) => {
                const areaT = normalizar(t.area);
                return areasParaBuscar.some((areaCap) => {
                    if (["mantenimiento", "taller", "mecanizacion"].some(k => areaCap.includes(k))) {
                        return ["mantenimiento", "taller", "mecanizacion"].some(k => areaT.includes(k));
                    }
                    return areaT.includes(areaCap);
                });
            });

            const dnisObjetivo = new Set(trabajadoresObjetivo.map((t) => t.dni?.trim()));
            const asistentesValidos = capacitacion.participantes.filter((p) => dnisObjetivo.has(p.dni?.trim()));
            const asistentesDNI = new Set(asistentesValidos.map((p) => p.dni?.trim()));

            faltantes = trabajadoresObjetivo.filter((t) => !asistentesDNI.has(t.dni?.trim()));

            cobertura.total_objetivo = trabajadoresObjetivo.length;
            cobertura.asistentes = asistentesDNI.size;
            cobertura.porcentaje = cobertura.total_objetivo > 0
    ...
    async update(id, data) {
        const { participantes, nuevosDocumentos, ...resto } = data;
        ...
        const mapaFirmas = new Map(antiguos.map(p => [p.dni?.trim(), p.firma_url]));
        ...
        await prisma.participantes.deleteMany({ where: { id_capacitacion: Number(id) } });
